...
namespace AbpProjects.Tests
{
    public abstract class AbpProjectsTestBase : AbpIntegratedTestBase<AbpProjectsTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected AbpProjectsTestBase()
        {
        ...
        protected void UsingDbContext(Action<AbpProjectsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AbpProjectsDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AbpProjectsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AbpProjectsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AbpProjectsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpProjectsDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AbpProjectsDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpProjectsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AbpProjectsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AbpProjectsDbContext, Task<T>> func)
        {
            T result;

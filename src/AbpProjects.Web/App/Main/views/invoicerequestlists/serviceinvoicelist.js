(function() {
    angular.module('app').controller('app.views.invoicerequestlists.serviceinvoicelist', [
        ...
        function($scope, $state, $timeout, $uibModal, projectService, uiGridConstants) {
            ...
            vm.save = function (t) {
                abp.message.confirm(
                    ...
                    function (isConfirmed) {
                        if (isConfirmed) {
                            projectService.updatetocancelservice(t.id)
                                .then(function () {
                                    abp.notify.info("Cancel: " + t.domainName);
                                    /* getDomainDate();*/
                                    // init();
                                    getInvoicerequestListByService();
            ...
            function getInvoicerequestListByService() {
                ...
                if (vm.task.domainName != "") {
                    vm.requestParams.domainName = vm.task.domainName;
                } else {
                    vm.requestParams.domainName = "";

                }
                abp.ui.setBusy();
                projectService.getInvoicerequestListByService($.extend({ Filter: vm.nameFilter }, vm.requestParams))
                ...
                columnDefs: [
                    {
                        ...
                        cellTemplate:
                            ...
                            '    <ul uib-dropdown-menu ng-show="row.entity.status!=2">' +
                            ...
                            '      <li><a ng-click="grid.appScope.save(row.entity)" ng-show="row.entity.status!=2">' + App.localize('Cancel') + '</a></li>' +
...

                    {
                        ...
                        cellTemplate: '<div class=\"ui-grid-cell-contents\" style=\"float: left\">' +
                            ' <div><p ng-if="row.entity.domainName!=null"><b>Domain Name: </b><span ng-if="row.entity.domainName!=null">{{row.entity.domainName}} </span></p><p ng-if="row.entity.hostingSpace!=null"><b>Hosting Space: </b><span ng-if="row.entity.hostingSpace!=null">{{row.entity.hostingSpace}} </span></p><p ng-if="row.entity.typeName != null"><b>Type Name:</b><span>{{row.entity.typeName}} </span></p><p ng-if="row.entity.noOfEmail!=null"><b>No of email: </b><span>{{row.entity.noOfEmail}} </span></p><p ng-if="row.entity.serverName!=null"><b>Server Type:</b><span>{{row.entity.serverName}} </span></p><p ng-if="row.entity.typeofssl!=null"><b>Type Of SSL: </b><span ng-if="row.entity.typeofssl!=null">{{row.entity.typeofssl}} </span></p><p ng-if="row.entity.title!=null"><b>Title: </b><span ng-if="row.entity.title!=null">{{row.entity.title}} </span></p><p ng-if="row.entity.credits!=null"><b>Credits:</b><span>{{row.entity.credits}} </span></p><p ng-if = "row.entity.databaseSpace!=null" ><b>Database Space:</b><span ng-if="row.entity.typeName!=null">{{row.entity.typeName}} </span></p></div>' +
                    ...
                    },
                    {
                        ...
                        cellTemplate: '<div class=\"ui-grid-cell-contents\" style=\"float: left\">' +
                            ' <span>{{row.entity.serviceName}}</span>' +
                    ...
                    //},
                    {
                        name: ' Invoice Note',
                        enableColumnMenu: false,
                        headerCellClass: 'cleftalign',
                        cellClass: 'leftalign',
                        field: 'comment',
                        /* cellFilter: 'momentFormat: \'DD/MM/YYYY\'',*/
                        cellTemplate: '<div class=\"ui-grid-cell-contents\">' +
                            ' <span>{{row.entity.comment}}</span>' +
                            '</div>',
                        width: 114,
                    },
                    {
...

                    {
                        ...
                        //    ' <span>{{row.entity.creationTime| date:dd/ MM / yyyy}}</span>' +

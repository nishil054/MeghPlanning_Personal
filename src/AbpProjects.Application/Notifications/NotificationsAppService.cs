...
{
    [AbpAuthorize]
    public class NotificationsAppService : AbpProjectsAppServiceBase, INotificationsAppService
    {
        ...
        public async Task<GetNotificationSettingsOutput> GetNotificationSettings()
        {
           ...
           output.ReceiveNotifications = Convert.ToBoolean(await SettingManager.GetSettingValueAsync(NotificationSettingNames.ReceiveNotifications));
           // var d=  SettingManager.GetAllSettingValuesForUser(AbpSession.ToUserIdentifier());
            //output.ReceiveNotifications = from k in d
            //                              where 
           // output.ReceiveNotifications = Convert.ToBoolean(d.Where(x => x.Name == NotificationSettingNames.ReceiveNotifications).Select(x => x.Value));
